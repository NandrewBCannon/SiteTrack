...
export function AssetCard({ asset, canDelete = false, onDelete }: { asset: AssetView; canDelete?: boolean; onDelete?: (asset: AssetView) => void }) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-[8px] border border-zinc-200 bg-white shadow-panel transition hover:-translate-y-0.5 hover:border-zinc-300">
      <Link href={`/assets/${asset.id}`} className="grid sm:grid-cols-[168px_1fr]">
        ...
        <div className="min-w-0 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3 pr-11 sm:pr-0">
            ...
            <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${statusClass(asset.status)}`}>
      ...
      <div className="absolute right-3 top-3">
        <AssetActionsMenu asset={asset} canDelete={canDelete && Boolean(onDelete)} onDelete={(item) => onDelete?.(item)} />
      </div>
    </article>
  );
}
